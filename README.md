...
현재 프런트 Golden Demo 화면, SECOM Detect 재현 파이프라인, FastAPI·PostgreSQL·Alembic 기반 Backend Workflow가 구현되어 있습니다. Backend에는 Detect artifact import, 최소 API key/role 경계, Passport Evidence 개발용 storage, versioned Rule policy catalog 기반도 포함됩니다. 고정 Golden R01 Match snapshot으로 전체 흐름을 재현하며 실제 BGE-M3·ChromaDB Adapter, SSO/tenant와 운영 object storage는 후속 통합 범위입니다. 실행 방법과 API 순서는 [`backend/README.md`](backend/README.md)를 참고합니다.
