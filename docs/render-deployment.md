...
- `/health/ready`가 DB, BGE/Chroma, Evidence bucket을 모두 확인해야 배포가 healthy입니다.
...
준비해야 하며 이 저장소에는 credential을 커밋하지 않습니다.
...
2. private S3-compatible bucket과 최소권한 object credential을 만듭니다.
3. `render blueprints validate render.yaml`로 Blueprint를 다시 검증합니다.
...
7. `/health/live`와 `/health/ready`를 확인합니다. ready 응답의 provider는
...
Chroma를 upsert합니다. `ADMIN` API key로 실제 수요처가 검증한 Demand를 최소 3건 등록합니다.
